...
def process_args():
    ...
    parser.add_argument("--dst", help="The output file")
    parser.add_argument("--topk", type=int, help="The number of base element to pick up")
...
if __name__ == "__main__":
    ...
    base_fname = '/your/path/to/gist1m_dataset'
    ...
    np.savetxt(res_save+"gist1M_faiss_kmeans_4partitions_Ids_belong_which_centroids.csv", I.astype(int), delimiter=",")
    np.savetxt(res_save+"gist1M_faiss_kmeans_4partitions_Ids_Distance_to_closest_centroids.csv", D, delimiter=",")
    np.save(res_save+"gist1M_faiss_kmeans_4partitions_centroids.npy", kmeans.centroids)

    # localize the distributed indexes
    index_path0=res_save+'faiss_kmeans_L2_index_gist1Mef200m16_0.bin'
    index_path1=res_save+'faiss_kmeans_L2_index_gist1Mef200m16_1.bin'
    index_path2=res_save+'faiss_kmeans_L2_index_gist1Mef200m16_2.bin'
    index_path3=res_save+'faiss_kmeans_L2_index_gist1Mef200m16_3.bin'
    ...
    with open(res_save+'partition0_2_base_L2_idx.txt', 'w') as fp:
        for item in dic0_id_2_base:
            ...
            fp.write("%s\n" % item)
        print('localized p0 to '+res_save+'partition0_2_base_L2_idx.txt')
    ...
    with open(res_save+'partition1_2_base_L2_idx.txt', 'w') as fp:
        for item in dic1_id_2_base:
            # write each item on a new line
            fp.write("%s\n" % item)
        print('localized p1 to '+res_save+'partition1_2_base_L2_idx.txt')
    ...
    with open(res_save+'partition2_2_base_L2_idx.txt', 'w') as fp:
        for item in dic2_id_2_base:
            # write each item on a new line
            fp.write("%s\n" % item)
        print('localized p2 to '+res_save+'partition2_2_base_L2_idx.txt')

    with open(res_save+'partition3_2_base_L2_idx.txt', 'w') as fp:
        for item in dic3_id_2_base:
            # write each item on a new line
            fp.write("%s\n" % item)
        print('localized p3 to '+res_save+'partition3_2_base_L2_idx.txt')