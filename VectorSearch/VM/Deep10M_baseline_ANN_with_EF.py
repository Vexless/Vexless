...
# get index path
DIM = 96
index_path0 = DEST_FILE+'Deep/deep1b_gt/index/index_Deep10Mef200.bin'


# get partitioned mapping
# dict0 = np.genfromtxt(DEST_FILE+'Deep/deep1b_gt/index/partition0_2_base_idx.txt', delimiter='\n',dtype=int)
# dict1 = np.genfromtxt(DEST_FILE+'Deep/deep1b_gt/index/partition1_2_base_idx.txt', delimiter='\n',dtype=int)
# dict2 = np.genfromtxt(DEST_FILE+'Deep/deep1b_gt/index/partition2_2_base_idx.txt', delimiter='\n',dtype=int)

# print(len(dict0),len(dict0),len(dict))
# get centroids, 3 * 128 dim vector (float)
# centroids = np.genfromtxt(DEST_FILE+'Deep/deep1b_gt/faiss_kmeans_index/Deep10M_3centroids.csv', delimiter=',')
# print(centroids.shape)

# centroids = [] # there are 3 centrioids in partitioned index.
# centroid = [] # temp var that stores each centroid 
# centroid = []
# with open(DEST_FILE+'centroids.txt') as f:
...
DEEP10M_PATH = '/path/to/file/'
# SIFT1M_CSV_PATH = blob_url.split('//')[-2]
# LOAD DATA
def read_vector_from_csv(vector_numpy_set_str_name):
    return np.loadtxt(DEEP10M_PATH+vector_numpy_set_str_name+'.csv', delimiter=",")
query = read_vector_from_csv('deep_query')
# base = read_vector_from_csv('base')
gt = read_vector_from_csv('deep1b_gt/deep_gt_10M')
