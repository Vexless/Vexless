...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    entity_ids = [df.EntityId("DEF", str(i)) for i in range(partition_cnt)] # Assuming you have 5 entities
        ...
        if vec_id_val is not None:
            ...
            for d in range(partition_cnt):
                ...
                if d_partition_cnt[d]<=Ts:
                    work_batch.append(d)
            parallel_tasks = [ context.call_entity(eid, "query", query) for eid in work_batch] # specifying DEF ids.
            for i, task in enumerate(parallel_tasks):
                logging.info(f"Parallel function #{i + 1} activated.")
            start_time = context.current_utc_datetime
            # outputs = yield context.task_all(parallel_tasks)
            outputs = [task.result for task in parallel_tasks if task.is_completed]
